export function buildSystemPrompt(farmSchema?: any): string {
...
export function readSensorFromSchema(sensor: string, farmSchema?: any): string {
...
export async function executeTool(toolName: string, args: any, baseUrl: string, farmSchema?: any): Promise<string> {
  try {
    switch (toolName) {
      case 'query_sensors': {
      ...
      case 'control_actuator': {
        // Log the intervention so Uma can recall it later
        const message = `Actuator command: ${args.target} ${args.id} ${args.action} - ${args.reason?.replace(/"/g, "'") || ''}`;
        await executeGraphQL(baseUrl, `mutation { logTelemetry(type: "intervention", value: 1, message: "${message}") { id } }`);
